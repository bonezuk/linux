...
#include "hifiberry_dacplus_coeff.h"
...
static int snd_rpi_hifiberry_dacplus_is_coefficients(void)
{
	...
	mutex_lock(&driver_data->dac_lock);
	for(i=0; i<12 && !is_coeff; i++)
	{
		if(driver_data->filters[i].is_active)
		{
			is_coeff = 1;
		}
	}
...
static int snd_rpi_hifiberry_dacplus_apply_coefficients(struct snd_soc_codec *codec,int fIndex)
{
	...
	res = snd_rpi_hifiberry_dacplus_load_default_coefficients(codec);
	if(!res)
		return res;
		...
		if(driver_data->filters[filterIndex].is_active)
		{
			for(coIndex = 0; coIndex < 5 && !res; coIndex++)
			{
				for(i = 0; i < 3 && !res; i++,reg++)
				{
					...
					if(!res)
						res = regmap_write(pcm512x->regmap,PCM512x_PAGE_BASE(page+18) + reg,(unsigned int)driver_data->filters[filterIndex].coeff[fIndex][coIndex][i]);
				}
				reg++;
			}
		}
                                             ...
                                             struct snd_pcm_substream *substream)
{
	...
	if(snd_rpi_hifiberry_dacplus_is_coefficients())
	{
		snd_rpi_hifiberry_dacplus_apply_coefficients(codec,5);
	}
