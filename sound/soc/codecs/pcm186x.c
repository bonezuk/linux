...
{
	return pcm1681_accessible_reg(dev, reg) && (reg != PCM1681_ZERO_DETECT_STATUS);
}
...
static int pcm1681_set_deemph(struct snd_soc_codec *codec)
{
	...
	if (val != -1) {
		regmap_update_bits(priv->regmap, PCM1681_DEEMPH_CONTROL, PCM1681_DEEMPH_RATE_MASK, val << 3);
		enable = 1;
	} else {
		enable = 0;
	}
	...
	return regmap_update_bits(priv->regmap, PCM1681_DEEMPH_CONTROL, PCM1681_DEEMPH_MASK, enable);
}

static int pcm1681_get_deemph(struct snd_kcontrol *kcontrol,struct snd_ctl_elem_value *ucontrol)
{
...
static int pcm1681_put_deemph(struct snd_kcontrol *kcontrol,struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct pcm1681_private *priv = snd_soc_codec_get_drvdata(codec);
...
static int pcm1681_set_dai_fmt(struct snd_soc_dai *codec_dai,unsigned int format)
{
...
static const struct snd_kcontrol_new pcm1681_controls[] = {
	...
	SOC_DOUBLE_R_TLV("Channel 7/8 Playback Volume",
		...
		0x7f, 0, pcm1681_dac_tlv),
	SOC_SINGLE_BOOL_EXT("De-emphasis Switch", 0,pcm1681_get_deemph, pcm1681_put_deemph),
};
...
static int pcm1681_i2c_probe(struct i2c_client *client,const struct i2c_device_id *id)
{
	int ret;
	struct pcm1681_private *priv;

	printk(KERN_INFO "pcm1681_i2c_probe\n");
	...
	return snd_soc_register_codec(&client->dev, &soc_codec_dev_pcm1681, &pcm1681_dai, 1);
}

static int pcm1681_i2c_remove(struct i2c_client *client)
{
	printk(KERN_INFO "pcm1681_i2c_remove\n");
